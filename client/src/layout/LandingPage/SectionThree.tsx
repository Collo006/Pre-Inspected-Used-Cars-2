...
export default function SectionThree(){
  ...
  const totalSlides = data?.suv_pickups?.length
    ? Math.ceil(data.suv_pickups.length / carsPerSlide)
    : 1;

    useEffect(() => {
    if (!data?.suv_pickups|| !scrollContainerRef.current) return;
        ...
        <div ref={scrollContainerRef} className="overflow-hidden w-full">
          <div className="flex w-full">
            {Array.from({ length: totalSlides }).map((_, slideIndex) => {
              const carsForThisSlide = data.suv_pickups.slice(
